...
interface Tier { id: string | null; name: string; sortOrder: number; minTeam: number; minEarningsCents: string }
...
export default function Ranks() {
  ...
  async function customize() {
    ...
    try {
      for (const t of data.tiers) {
        await api.post('/admin/ranks', { name: t.name, sortOrder: t.sortOrder, minTeam: t.minTeam, minEarningsCents: Number(t.minEarningsCents) });
      }
  ...
  async function addTier() {
    ...
    try { await api.post('/admin/ranks', { name: 'New tier', sortOrder: order, minTeam: 0, minEarningsCents: 0 }); await load(); }
    catch (e) { setError(String((e as ApiError).message)); }
  }
  async function saveTier(t: Tier) {
    try { await api.patch(`/admin/ranks/${t.id}`, { name: t.name, sortOrder: t.sortOrder, minTeam: t.minTeam, minEarningsCents: Number(t.minEarningsCents) }); showToast('Saved'); }
    catch (e) { setError(String((e as ApiError).message)); }
  }
    ...
    <div className="card" style={{ maxWidth: 640 }}>
      <div className="spread" style={{ marginBottom: 12 }}>
        <div>
          ...
          <div className="faint" style={{ fontSize: 12 }}>Tiers by team size + cumulative earnings. Members see their rank and progress.</div>
        </div>
      ...
      <table>
        <thead><tr><th>Name</th><th>Min team</th><th>Min earnings ($)</th><th></th></tr></thead>
        <tbody>
          {data.tiers.map((t, i) => (
            <tr key={t.id ?? i}>
