...
import { Wallet, Banknote, Users, CheckCircle2, Sparkles, ArrowRight, AlertTriangle, Send } from 'lucide-react';
  ...
  return (
    <div>
      ...
      <div className="stat-grid fade-in delay-1" style={{ marginBottom: 16 }}>
        {earn && <StatCard label="Earned (this month)" value={<MoneyCounter cents={Number(earn.earnedThisMonthCents)} currency={earn.currency} />} icon={<Wallet className="size-[18px]" aria-hidden />} grad="var(--foil)" hint={`${money(earn.soldThisMonthCents, earn.currency)} sold this month`} />}
        {earn && <StatCard label="Sold (lifetime)" value={<MoneyCounter cents={Number(earn.soldLifetimeCents)} currency={earn.currency} />} icon={<Banknote className="size-[18px]" aria-hidden />} />}
        <StatCard label={t('me.members')} value={<CountUp value={team.totalMembers} />} icon={<Users className="size-[18px]" aria-hidden />} grad="var(--grad-primary)" />
        <StatCard label={t('me.activeMembers')} value={<CountUp value={team.totalActive} />} icon={<CheckCircle2 className="size-[18px]" aria-hidden />} grad="var(--grad-emerald)" />
      </div>
      ...
      {recruits && (
        <div className="card lift fade-in delay-3" style={{ marginTop: 16 }}>
          <div className="spread" style={{ alignItems: 'flex-start', marginBottom: 4 }}>
            ...
            <Link href="/app/invite" className="btn sm"><Sparkles className="size-4" aria-hidden />Invite</Link>
          </div>
          ...
          {recruits.summary.needsNudgeCount > 0 && (
            <Alert style={{ margin: '8px 0 12px', background: 'color-mix(in srgb, var(--amber) 12%, transparent)', borderColor: 'color-mix(in srgb, var(--amber) 32%, transparent)' }}>
              <AlertDescription className="row" style={{ gap: 8, fontSize: 13, alignItems: 'flex-start' }}>
                <AlertTriangle className="size-4" aria-hidden style={{ flexShrink: 0, marginTop: 1, color: 'var(--amber)' }} />
          ...
          {recruits.recruits.length === 0 ? (
            <div className="muted" style={{ textAlign: 'center', padding: '22px 0' }}>
              ...
              <Link href="/app/invite" className="btn sm" style={{ marginTop: 12, display: 'inline-flex', alignItems: 'center', gap: 6 }}>Send your first invite<ArrowRight className="size-4" aria-hidden /></Link>
            </div>
          ) : (
                  ...
                  {recruits.recruits.map((r) => (
                    <tr key={r.id} style={r.needsNudge ? { background: 'color-mix(in srgb, var(--amber) 6%, transparent)' } : undefined}>
                      ...
                      <td style={{ textAlign: 'right' }}>
                        {r.needsNudge && <button className="btn ghost sm" onClick={() => nudge(r)} title={`Copy ${r.fullName}'s email`}><Send className="size-4" aria-hidden />Nudge</button>}
                      </td>
                    </tr>
                  ))}
                </tbody>
