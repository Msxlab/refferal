...
export function AuditPageContent({ tenantName }: { tenantName: string }) {
  void tenantName;
  ...
  if (error && !list) return <div className="error" style={{ margin: 24 }}>{error} <Button variant="ghost" size="sm" onClick={() => void load()} style={{ marginLeft: 8 }}>Retry</Button></div>;
    ...
    <div>
      <div className="spread">
        ...
        <div className="row fade-in no-print" style={{ gap: 8 }}>
          {integrity && (
            <Badge variant={integrity.ok ? 'success' : 'destructive'} title={integrity.brokenAt ? `Broken at ${integrity.brokenAt}` : undefined}>
              {integrity.ok ? `✓ Chain intact (${integrity.checked})` : '✗ Chain tampered'}
            </Badge>
          )}
          <Button variant="ghost" onClick={verifyIntegrity} disabled={verifying}>{verifying ? 'Verifying…' : '🔒 Verify integrity'}</Button>
      ...
      <div className="row fade-in delay-1 no-print" style={{ gap: 10, flexWrap: 'wrap', alignItems: 'center', margin: '14px 0' }}>
        <Input name="audit-search" aria-label="Search audit log by action or entity" placeholder="🔍  Search action or entity…" value={q} onChange={(e) => { setQ(e.target.value); setPage(1); }} style={{ flex: 1, minWidth: 180, maxWidth: 280 }} />
        <select name="audit-entity" value={entity} onChange={(e) => { setEntity(e.target.value); setPage(1); }} style={{ width: 'auto' }} aria-label="Entity">
          <option value="">All entities</option>
          {ENTITIES.map((e) => <option key={e} value={e}>{ICON[e] ?? '•'} {e}</option>)}
        </select>
        <Input type="date" name="audit-from" value={from} onChange={(e) => { setFrom(e.target.value); setPage(1); }} aria-label="From" className="w-auto" />
        <Input type="date" name="audit-to" value={to} onChange={(e) => { setTo(e.target.value); setPage(1); }} aria-label="To" className="w-auto" />
        <span style={{ flex: 1 }} />
          ...
          <div>
            {items.map((a) => (
              <button key={a.id} onClick={() => setDetail(a)} className="row inbox-row" style={{ alignItems: 'flex-start' }}>
                <span style={{ width: 32, height: 32, borderRadius: 9, display: 'grid', placeItems: 'center', background: a.entity === 'security' ? 'color-mix(in srgb, var(--rose) 16%, transparent)' : 'var(--panel-2)', fontSize: 15, flexShrink: 0 }}>
                  {ICON[a.entity] ?? '•'}
                </span>
