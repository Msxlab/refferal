...
type BadgeVariant = 'default' | 'secondary' | 'success' | 'destructive' | 'pending' | 'payable' | 'outline';
const ST: Record<string, BadgeVariant> = {
  active: 'success', inactive: 'secondary', draft: 'secondary', approved: 'success', void: 'destructive',
  pending: 'pending', payable: 'payable', paid: 'success', reversed: 'destructive', expired: 'secondary', used: 'success',
};
const stVar = (s: string): BadgeVariant => ST[s] ?? 'secondary';
    ...
    <div>
      <div className="spread">
        <div>
          ...
          <p className="sub fade-in">Invite members, assign roles, deactivate. Placement is permanent.
            {nps && nps.total > 0 && nps.nps != null && <Badge variant="success" className="ml-2">NPS {nps.nps} · {nps.total} responses</Badge>}
            {funnel && funnel.views > 0 && <Badge variant="payable" className="ml-2">Invite funnel: {funnel.views} views → {funnel.signups} signups{funnel.conversionPct != null ? ` (${funnel.conversionPct}%)` : ''}</Badge>}
          </p>
        </div>
            ...
            <tbody>
              {list.items.map((m) => (
                <tr key={m.id} style={{ cursor: 'pointer', background: selected.has(m.id) ? 'var(--panel-2)' : undefined }} onClick={() => setDetailId(m.id)}>
                  ...
                  {cols.isVisible('code') && <td style={{ fontFamily: 'ui-monospace, monospace' }} onClick={(e) => e.stopPropagation()}><span className="row" style={{ gap: 4 }}>{m.referralCode}<Button variant="ghost" size="sm" title="Copy code" className="h-7 min-w-0 px-2 text-[11px]" onClick={() => { navigator.clipboard.writeText(m.referralCode).then(() => showToast('Copied ✓')).catch(() => {}); }}>⧉</Button></span></td>}
                  {cols.isVisible('sponsor') && (
                    <td onClick={(e) => e.stopPropagation()}>
                      {m.sponsorReferralCode
                        ? <Button variant="ghost" size="sm" className="h-7 px-2 font-mono" onClick={() => openSponsor(m.sponsorReferralCode!)}>{m.sponsorReferralCode}</Button>
                  ...
                  {cols.isVisible('status') && <td><Badge variant={stVar(m.status)}>{m.status}</Badge></td>}
      ...
      {showInvite && (
        <Modal title="Invite a member" onClose={() => setShowInvite(false)}>
          <form onSubmit={invite} className="w-full">
            <div className="mb-3.5">
              <Label htmlFor={`${uid}-sponsor`} className="mb-1.5 block">Sponsor referral code (blank = yourself)</Label>
              <Input id={`${uid}-sponsor`} value={sponsor} onChange={(e) => setSponsor(e.target.value)} placeholder="e.g. ALICE1" autoFocus />
            </div>
            {error && <div className="mb-2 text-sm text-destructive">{error}</div>}
            {latest ? (
              <Card className="mt-1 p-3" style={{ background: 'color-mix(in srgb, var(--sky) 8%, transparent)' }}>
                ...
                <div className="row spread" style={{ gap: 8 }}>
                  ...
                  <Button type="button" variant="ghost" size="sm" onClick={() => { navigator.clipboard.writeText(inviteUrl); showToast('Copied ✓'); }}>Copy</Button>
                </div>
              </Card>
            ) : null}
      ...
      {showAdd && (
        <Modal title="Add member" onClose={() => setShowAdd(false)}>
          <div className="w-full">
            {addResult ? (
              <div>
                <Card className="p-3.5" style={{ background: 'color-mix(in srgb, var(--emerald) 10%, transparent)', borderColor: 'color-mix(in srgb, var(--emerald) 30%, transparent)' }}>
                  ...
                  {addResult.tempPassword ? (
                    <div style={{ marginTop: 8 }}>
                      ...
                      <div className="row spread" style={{ gap: 8 }}>
                        <span style={{ color: 'var(--gold-500)', fontFamily: 'ui-monospace, monospace', fontWeight: 700 }}>{addResult.tempPassword}</span>
                        <Button variant="ghost" size="sm" onClick={() => { navigator.clipboard.writeText(addResult.tempPassword!).then(() => showToast('Copied ✓')).catch(() => {}); }}>Copy</Button>
                      </div>
                    </div>
              ...
              <form onSubmit={createMember}>
                <div className="mb-3.5"><Label htmlFor={`${uid}-addname`} className="mb-1.5 block">Full name</Label><Input id={`${uid}-addname`} value={addName} onChange={(e) => setAddName(e.target.value)} required minLength={2} autoFocus placeholder="e.g. Jane Smith" /></div>
      ...
      {editM && (
        <Modal title={`Edit profile — ${editM.fullName}`} onClose={() => setEditM(null)}>
          <form onSubmit={saveProfile} className="w-full">
            <div className="mb-3.5"><Label htmlFor={`${uid}-editname`} className="mb-1.5 block">Full name</Label><Input id={`${uid}-editname`} value={editName} onChange={(e) => setEditName(e.target.value)} required minLength={2} autoFocus /></div>
            <div className="mb-3.5"><Label htmlFor={`${uid}-editemail`} className="mb-1.5 block">Email</Label><Input id={`${uid}-editemail`} type="email" value={editEmail} onChange={(e) => setEditEmail(e.target.value)} required /></div>
      ...
      {!d || !p ? <Loading rows={4} /> : (
        <div className="grid" style={{ gap: 18 }}>
          <div className="flex flex-wrap items-center gap-1.5">
            <Badge variant={stVar(p.status)}>{p.status}</Badge>
            <Badge variant="payable">{p.role.replace('tenant_', '')}</Badge>
            {p.emailVerified && <Badge variant="success">email verified</Badge>}
          ...
          {p.sponsor && (
            <div>
              ...
              <Button variant="ghost" size="sm" onClick={() => onNavigate(p.sponsor!.membershipId)}>
          ...
          <Section title="Recent sales">
            {d.recentSales.length === 0 ? <Empty /> : (
              <table>
                ...
                <tbody>
                  {d.recentSales.map((s) => (
                    <tr key={s.id}><td className="muted">{dateShort(s.saleDate)}</td><td className="tnum">{money(s.amountCents, cur)}</td><td><Badge variant={stVar(s.status)}>{s.status}</Badge></td></tr>
                  ))}
                </tbody>
              </table>
            )}
                ...
                <tbody>
                  {d.recentLedger.map((e) => (
                    <tr key={e.id}>
                      ...
                      <td><Badge variant={stVar(e.status)}>{e.status}</Badge></td>
