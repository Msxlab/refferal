    ...
    return (
      <div>
        <div className="row" style={{ gap: 10, marginBottom: 12, alignItems: 'center' }}>
          <Button variant="ghost" size="sm" onClick={() => { setRoot(null); setNodes(null); }}>← Leaders</Button>
            ...
            <div>
              ...
              <div className="row" style={{ gap: 8, flexWrap: 'wrap' }}>
                {health.dormantClusters.map((d) => (
                  <Button key={d.leaderId} variant="ghost" size="sm" onClick={() => openTree(d.leaderId, d.leaderName)} title={`${d.referralCode} · team of ${d.teamSize}`}>
                  ...
                  <button key={l.id} className="card hover" style={{ textAlign: 'left', cursor: 'pointer' }} onClick={() => openTree(l.id, l.fullName)}>
                    <div className="spread" style={{ alignItems: 'flex-start' }}>
                      ...
                      <div className="row" style={{ gap: 6, flexShrink: 0 }}>
                        {l.isOwnerRoot ? <Badge variant="success">owner</Badge> : <Badge variant="payable">🎖</Badge>}
                        <StatusPill s={st} />
                      </div>
                    </div>
