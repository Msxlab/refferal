    ...
    <div>
      ...
      <div className="row fade-in delay-1 no-print" style={{ gap: 10, flexWrap: 'wrap', alignItems: 'center', margin: '14px 0' }}>
        ...
        <Popover label={<>Filters</>} badge={advCount} width={300}>
          {(close) => (
            <div className="grid" style={{ gap: 12 }}>
              <div>
                <Label htmlFor={`${uid}-fstatus`} className="mb-1.5 block">Status</Label>
                <select id={`${uid}-fstatus`} value={filters.status} onChange={(e) => patchFilters({ ...filters, status: e.target.value })}>
                  {STATUSES.map((s) => <option key={s} value={s}>{s || 'All statuses'}</option>)}
                </select>
              </div>
              <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div><Label htmlFor={`${uid}-ffrom`} className="mb-1.5 block">From</Label><Input id={`${uid}-ffrom`} type="date" value={filters.from} onChange={(e) => patchFilters({ ...filters, from: e.target.value })} /></div>
                <div><Label htmlFor={`${uid}-fto`} className="mb-1.5 block">To</Label><Input id={`${uid}-fto`} type="date" value={filters.to} onChange={(e) => patchFilters({ ...filters, to: e.target.value })} /></div>
                <div><Label htmlFor={`${uid}-fmin`} className="mb-1.5 block">Min ($)</Label><Input id={`${uid}-fmin`} type="number" min={0} step="0.01" value={filters.minCents ? String(Number(filters.minCents) / 100) : ''} onChange={(e) => patchFilters({ ...filters, minCents: e.target.value ? String(Math.round(parseFloat(e.target.value) * 100)) : '' })} placeholder="0.00" /></div>
                <div><Label htmlFor={`${uid}-fmax`} className="mb-1.5 block">Max ($)</Label><Input id={`${uid}-fmax`} type="number" min={0} step="0.01" value={filters.maxCents ? String(Number(filters.maxCents) / 100) : ''} onChange={(e) => patchFilters({ ...filters, maxCents: e.target.value ? String(Math.round(parseFloat(e.target.value) * 100)) : '' })} placeholder="0.00" /></div>
              </div>
        ...
        {views.map((v) => (
          <span key={v.id} className="row" style={{ gap: 3 }}>
            <Button variant="ghost" size="sm" onClick={() => applyView(v)} title={v.mine ? undefined : `Shared by ${v.ownerName ?? 'team'}`}>
              {v.shared && <span style={{ marginRight: 3 }}>👥</span>}{v.name}
            </Button>
            {v.mine && <button className="text-xs text-muted-foreground hover:text-foreground" onClick={() => deleteView(v.id)} aria-label={`Delete ${v.name}`}>✕</button>}
          </span>
        ))}
        ...
        <Button variant="ghost" size="sm" onClick={() => { setViewName(''); setViewShared(false); setShowSaveView(true); }}>＋ Save view</Button>
      </div>
                  ...
                  {cols.isVisible('seller') && (
                    <td>
                      <span className="row" style={{ gap: 6 }}>
                        {s.sellerName}
                        {s.selfSubmitted && <Badge title="Submitted by member">self</Badge>}
                      </span>
                  ...
                  {cols.isVisible('status') && (
                    <td>
                      <span className="inline-flex items-center gap-1.5">
                        ...
                        {s.deliveredAt && <Badge variant="success">✓ delivered</Badge>}
                      </span>
                    </td>
                  )}
      ...
      {showNew && (
        <Modal title="Record a sale" onClose={() => setShowNew(false)}>
          <form onSubmit={createSale} className="w-full">
            <div className="relative mb-3.5">
              <Label htmlFor={`${uid}-seller`} className="mb-1.5 block">{t('sales.seller')}</Label>
              <Input id={`${uid}-seller`} value={code} onChange={(e) => { setCode(e.target.value); setSellerPicked(false); }} placeholder="Search name or code…" required autoFocus autoComplete="off" />
              {sellerOpts.length > 0 && !sellerPicked && (
                <div className="absolute left-0 right-0 top-full z-10 max-h-[200px] overflow-y-auto rounded-xl border border-border bg-popover p-1 shadow-lg">
                  {sellerOpts.map((o) => (
                    <button key={o.referralCode} type="button" className="row spread w-full cursor-pointer rounded-lg px-2.5 py-[7px] text-left hover:bg-accent"
            ...
            <div className="mb-3.5">
              <Label htmlFor={`${uid}-amount`} className="mb-1.5 block">{t('sales.amount')} ($)</Label>
              <Input id={`${uid}-amount`} type="number" min={0} step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="100000.00" required />
              {Number(amount) > 0 && <div className="faint" style={{ fontSize: 12, marginTop: 4 }}>= {money(Math.round(parseFloat(amount) * 100), cur)}</div>}
            </div>
            <div className="flex gap-3">
              <div className="flex-1"><Label htmlFor={`${uid}-saledate`} className="mb-1.5 block">Sale date</Label><Input id={`${uid}-saledate`} type="date" value={newDate} onChange={(e) => setNewDate(e.target.value)} /></div>
              <div className="flex-1"><Label htmlFor={`${uid}-cust`} className="mb-1.5 block">Customer (optional)</Label><Input id={`${uid}-cust`} value={newCustomer} onChange={(e) => setNewCustomer(e.target.value)} placeholder="e.g. Smith kitchen" /></div>
            </div>
            <div className="mt-3"><Label htmlFor={`${uid}-extref`} className="mb-1.5 block">External ref (optional)</Label><Input id={`${uid}-extref`} value={newExternalRef} onChange={(e) => setNewExternalRef(e.target.value)} placeholder="e.g. INV-2026-014" /></div>
      ...
      {showSaveView && (
        <Modal title="Save view" onClose={() => setShowSaveView(false)}>
          <form onSubmit={saveView} className="w-full">
            <div className="mb-3"><Label htmlFor={`${uid}-viewname`} className="mb-1.5 block">View name</Label><Input id={`${uid}-viewname`} value={viewName} onChange={(e) => setViewName(e.target.value)} placeholder="e.g. Awaiting approval" required autoFocus /></div>
      ...
      {!d ? <Loading rows={4} /> : (
        <div className="grid" style={{ gap: 18 }}>
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {d.deliveredAt && <Badge variant="success">delivered</Badge>}
            {d.selfSubmitted && <Badge>self-submitted</Badge>}
          </div>
