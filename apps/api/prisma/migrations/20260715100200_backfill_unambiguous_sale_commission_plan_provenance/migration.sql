...
trusted_winners AS (
  SELECT winner.sale_id, winner.tenant_id, winner.plan_id
  FROM as_of_winners winner
  WHERE winner.depth > 0
    AND winner.plan_updated_at < winner.evidence_at
    AND NOT EXISTS (
      SELECT 1
      FROM commission_plans later_higher
      WHERE later_higher.tenant_id = winner.tenant_id
        AND later_higher.created_at >= winner.evidence_at
        AND later_higher.effective_from <= winner.sale_date
        AND later_higher.effective_from > winner.effective_from
    )
    AND (
      SELECT COUNT(*)
      FROM commission_plan_levels snapshot_count
      WHERE snapshot_count.plan_id = winner.plan_id
    ) = winner.depth
    AND NOT EXISTS (
      SELECT 1
      FROM commission_plan_levels snapshot_level
      WHERE snapshot_level.plan_id = winner.plan_id
        AND (
          snapshot_level.level < 0
          OR snapshot_level.level >= winner.depth
          OR snapshot_level.created_at >= winner.evidence_at
          OR snapshot_level.updated_at >= winner.evidence_at
        )
    )
    AND NOT EXISTS (
      SELECT 1
      FROM ledger_entries le
      WHERE le.sale_id = winner.sale_id
        AND le.type = 'commission'
        AND (
          le.tenant_id IS DISTINCT FROM winner.tenant_id
          OR le.level < 0
          OR le.level >= winner.depth
          OR le.amount_cents <= 0
          OR FLOOR((winner.amount_cents::numeric * le.rate_bps_used::numeric) / 10000)
             IS DISTINCT FROM le.amount_cents::numeric
          OR NOT EXISTS (
            SELECT 1
            FROM commission_plan_levels matched_level
            WHERE matched_level.plan_id = winner.plan_id
              AND matched_level.level = le.level
              AND matched_level.rate_bps = le.rate_bps_used
              AND matched_level.created_at < winner.evidence_at
              AND matched_level.updated_at < winner.evidence_at
          )
        )
    )
),
