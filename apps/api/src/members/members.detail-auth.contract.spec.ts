...
describe("members fine-grained authorization contract", () => {
  it.each([
    ...
    expect(
      Reflect.getMetadata(
        PERMISSION_KEY,
        MembersAdminController.prototype[method],
      ),
    ).toBe(permission);
  });
  ...
  it("requires both structural and financial network permissions for leaders", () => {
    expect(
      Reflect.getMetadata(
        PERMISSION_KEY,
        MembersAdminController.prototype.leaders,
      ),
    ).toEqual(["network.view", "network.financials.view"]);
  });
  ...
  it("authorizes bulk status and role actions against their distinct permissions", () => {
    ...
    const controller = new MembersAdminController(
      members as never,
      null as never,
    );
    ...
    expect(
      Reflect.getMetadata(
        PERMISSION_KEY,
        MembersAdminController.prototype.bulk,
      ),
    ).toBeUndefined();
    expect(() =>
      controller.bulk(user(["members.suspend"]), { action: "activate", ids }),
    ).not.toThrow();
    expect(() =>
      controller.bulk(user(["members.suspend"]), { action: "deactivate", ids }),
    ).not.toThrow();
    expect(() =>
      controller.bulk(user(["members.suspend"]), {
        action: "set_role",
        ids,
        role: "tenant_staff",
      }),
    ...
    expect(() =>
      controller.bulk(user(["settings.roles"]), {
        action: "set_role",
        ids,
        role: "tenant_staff",
      }),
  ...
  it("requires role-management permission before manual creation can elevate a member", () => {
    ...
    expect(() =>
      controller.createManual(user(["members.manage"]), {
        ...base,
        role: "member",
      }),
    ).not.toThrow();
    ...
    expect(() =>
      controller.createManual(user(["members.manage", "settings.roles"]), {
        ...base,
        role: "tenant_admin",
      }),
    ).not.toThrow();
