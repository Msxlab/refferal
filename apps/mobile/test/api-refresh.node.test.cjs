...
function createControlledAsyncStorage() {
  ...
  return {
    ...
    failNextGet(error = new Error('get failed')) {
      getError = error;
    },
    failNextSet(error = new Error('set failed')) {
      setError = error;
    },
    failNextRemove(error = new Error('remove failed')) {
      removeError = error;
    },
    seedRaw(value) {
      stored = value;
...
async function assertRawBodyReplay({ label, body, contentType, assertBody }) {
  const owner = makeSession({
    accessToken: `${label}-expired-access`,
    refreshToken: `${label}-captured-refresh`,
  });
  const fresh = makeSession({
    accessToken: `${label}-fresh-access`,
    refreshToken: `${label}-rotated-refresh`,
  });
  ...
  try {
    assert.deepEqual(
      await loaded.api.request(`/replay-${label}`, {
        method: 'POST',
    ...
    assert.deepEqual(
      ...
      [`Bearer ${owner.accessToken}`, `Bearer ${fresh.accessToken}`],
    );
    assert.deepEqual(
      attempts.map((attempt) => attempt.contentType),
      [contentType, contentType],
    );
    await assertBody(attempts.map((attempt) => attempt.body));
  } finally {
...
test('session boundaries invalidate lineage and malformed or empty credentials fail closed', async () => {
  ...
  for (const raw of [
    ...
    assert.equal(storage.storedRaw(), null);
  }

  const failedReadStorage = createControlledAsyncStorage();
  failedReadStorage.seedSession(owner);
  ...
  await assert.rejects(
    invalidSaveAuth.saveSession({ ...owner, refreshToken: '' }),
    /invalid session/,
  );
  assert.equal(invalidSaveStorage.storedRaw(), null);

  const failedSaveStorage = createControlledAsyncStorage();
  failedSaveStorage.failNextSet();
  const failedSaveAuth = loadActualAuth(failedSaveStorage);
  await assert.rejects(failedSaveAuth.saveSession(owner), /set failed/);
  assert.equal(await failedSaveAuth.loadSession(), null);
  assert.equal(failedSaveStorage.storedRaw(), null);

  const failedClearStorage = createControlledAsyncStorage();
  const failedClearAuth = loadActualAuth(failedClearStorage);
  await failedClearAuth.saveSession(owner);
  failedClearStorage.failNextRemove();
  await assert.rejects(failedClearAuth.clearSession(), /remove failed/);
  assert.equal(await failedClearAuth.loadSession(), null);

  const invalidLoaded = loadApi({ ...owner, accessToken: '' });
  const previousFetch = globalThis.fetch;
  const observedAuthorizations = [];
  globalThis.fetch = async (_input, init) => {
    observedAuthorizations.push(authorization(init));
    return Response.json({ anonymous: true });
  };
  try {
    assert.deepEqual(await invalidLoaded.api.get('/invalid-stored-session'), { anonymous: true });
    assert.deepEqual(observedAuthorizations, [null]);
    assert.equal(invalidLoaded.persistence.clearCalls(), 0);
  } finally {
    globalThis.fetch = previousFetch;
  }
});
...
test('parallel 401 operations share one refresh and retry once with the same new session', async () => {
  ...
  const requestContentTypes = new Map();
  const refreshBodies = [];
  ...
  globalThis.fetch = async (input, init) => {
    ...
    const bodies = requestBodies.get(url) ?? [];
    bodies.push(init?.body);
    requestBodies.set(url, bodies);
    const contentTypes = requestContentTypes.get(url) ?? [];
    contentTypes.push(new Headers(init?.headers).get('Content-Type'));
    requestContentTypes.set(url, contentTypes);
    if (authorization(init) === `Bearer ${owner.accessToken}`) {
      unauthorizedCalls += 1;
  ...
  try {
    ...
    assert.deepEqual(requestBodies.get(`${apiBase}/second-resource`), [
      JSON.stringify({ value: 2 }),
      JSON.stringify({ value: 2 }),
    ]);
    assert.deepEqual(requestContentTypes.get(`${apiBase}/second-resource`), [
      'application/json',
      'application/json',
    ]);
  } finally {
  ...
  await assertRawBodyReplay({
    label: 'csv',
    body: 'member,amount\nA,42',
    contentType: 'text/csv',
    assertBody: async (bodies) => {
      assert.deepEqual(bodies, ['member,amount\nA,42', 'member,amount\nA,42']);
    },
  });
  const blob = new Blob(['binary-ledger'], { type: 'application/octet-stream' });
  await assertRawBodyReplay({
    label: 'blob',
    body: blob,
    contentType: null,
    assertBody: async (bodies) => {
      assert.equal(bodies[0], blob);
      assert.equal(bodies[1], blob);
      assert.deepEqual(await Promise.all(bodies.map((body) => body.text())), [
        'binary-ledger',
        'binary-ledger',
      ]);
    },
...
test('refresh parse failure clears once and normalizes every waiter to unauthorized', async () => {
  ...
  await assertConcurrentRefreshFailure((owner) =>
    Response.json({ ...owner, accessToken: '' }),
  );
  await assertConcurrentRefreshFailure((owner) =>
    Response.json({ ...owner, refreshToken: '' }),
  );
  await assertConcurrentRefreshFailure(() =>
    Response.json(
      makeSession({
        accessToken: 'wrong-owner-access',
        refreshToken: 'wrong-owner-refresh',
        userId: 'wrong-owner',
        membershipId: 'wrong-owner-membership',
        tenantId: 'wrong-owner-tenant',
      }),
    ),
  );
});
